...
enable_3rr = st.sidebar.checkbox("Enable Round Reversal", value=False)
reversal_round = None
if enable_3rr:
    reversal_round = st.sidebar.number_input("Reversal Round", min_value=2, max_value=total_rounds, value=3, step=1)
...
# Any change to these inputs invalidates the current simulation results
player_data = get_data(adp_file, proj_file, scoring_rules, roster_settings, league_size)

# --- SESSION STATE INITIALIZATION & INVALIDATION ---
# If the underlying data changes, clear the sim results
if 'last_scoring' not in st.session_state or st.session_state.last_scoring != scoring_rules:
    st.session_state.sim_results = None
    st.session_state.last_scoring = scoring_rules.copy()

if 'last_roster' not in st.session_state or st.session_state.last_roster != roster_settings:
    st.session_state.sim_results = None
    st.session_state.last_roster = roster_settings.copy()

if 'draft_state' not in st.session_state or st.sidebar.button("Reset Draft"):
    if not player_data.empty:
        st.session_state.draft_state = DraftState(league_size, total_rounds, player_data, reversal_round)
        st.session_state.sim_results = None
    else:
...
if draft_state:
    ...
    with col2:
        ...
        if st.button("Draft Player"):
            ...
            st.session_state.sim_results = None # Invalidate sim on pick
            st.rerun()
...
if not draft_state or draft_state.available_players.empty:
    ...
    # On-Demand Simulation Trigger
    if st.button(f"🚀 Run Probability Simulation ({num_simulations}x)"):
        with st.spinner("Simulating draft possibilities..."):
            st.session_state.sim_results = run_monte_carlo_simulations(
                draft_state, 
                draft_position, 
                roster_settings, 
    ...
    if 'sim_results' in st.session_state and st.session_state.sim_results is not None and not st.session_state.sim_results.empty:
        ...
        display_df = st.session_state.sim_results[display_cols].sort_values(by='VORP', ascending=False).reset_index(drop=True)
        ...
        # Default View (No Simulation)
        display_cols = ['Player', 'Position', 'Team', 'ADP', 'Projected_Points', 'VORP']
        display_df = draft_state.available_players[display_cols].sort_values(by='VORP', ascending=False).reset_index(drop=True)
